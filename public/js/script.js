$(function () {
	...
	var searchTextBox = $('.all-products input[type=text]');

	searchTextBox.change(function (event){
	...
	// Get data about our products from movies.json.
	...
	// It recieves one parameter - the data we took from movies.json.
		...
		if(data.length){
			data.forEach(function (item) {
				if(item.id == index){
					...
					container.find('label').text("Rating: "+ item.specs.rating);
	...
	function renderSearchResults(filter){
		...
		filter = (parseInt(filter)) ? parseInt(filter) : filter;

		products.forEach(function(product){
      if(typeof filter == 'number'){
        if(product.specs['year'] == filter){
	...
	function renderFilterResults(filters, products){
		...
		var criteria = ['manufacturer','year'],
			results = [],
			isFiltered = false;
